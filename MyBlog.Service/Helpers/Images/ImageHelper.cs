...
namespace MyBlog.Service.Helpers.Images
{
    public class ImageHelper : IImageHelper
    {
        ...
        public async Task<ImageUploadedDto> Upload(string? name, IFormFile? imageFile, ImageType? imageType, string folderName = null)
        {
            ...
            await using var stream = new FileStream(path, FileMode.Create, FileAccess.Write, FileShare.None, 1024 * 1024, useAsync : true);
            await imageFile.CopyToAsync(stream);
            await stream.FlushAsync();

            string message = imageType == ImageType.User ? $"{newFileName} isimli kullanıcı resmi eklenmiştir" : $"{newFileName} isimli makale resmi eklenmiştir";
